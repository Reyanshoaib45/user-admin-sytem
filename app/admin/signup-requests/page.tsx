...
import { ArrowLeft, Search, UserPlus, Check, X, Phone, Mail, Gift } from "lucide-react"
...
function AdminSignupRequestsPageContent() {
  ...
  const handleApproveRequest = () => {
    ...
    const newUser: User = {
      ...
      isOnboarded: false,
      referredBy: selectedRequest.referredBy,
      referralCode: `${newUserData.name.replace(/\s+/g, "").toUpperCase().slice(0, 4)}${Math.floor(Math.random() * 9999)
        .toString()
        .padStart(4, "0")}`,
    }
    ...
    if (selectedRequest.referredBy && selectedRequest.referralCode) {
      const referral = state.referrals.find(
        (ref) => ref.referralCode === selectedRequest.referralCode && ref.referredEmail === selectedRequest.email,
      )
      if (referral) {
  ...
  const handleRejectRequest = (requestId: string, reason: string) => {
    ...
    // If this was a referral, update the referral record
    if (request.referredBy && request.referralCode) {
      const referral = state.referrals.find(
        (ref) => ref.referralCode === request.referralCode && ref.referredEmail === request.email,
      )
      if (referral) {
        const updatedReferral = {
          ...referral,
          status: "pending" as const, // Reset to pending since signup was rejected
        }
        dispatch({ type: "UPDATE_REFERRAL", payload: updatedReferral })
      }
    }
  ...
  const getReferrerName = (referredBy: string) => {
      ...
      <main className="max-w-7xl mx-auto py-4 lg:py-8 px-4 sm:px-6 lg:px-8">
        <Card className="shadow-xl border-0 overflow-hidden">
          <CardHeader className="bg-gradient-to-r from-purple-500 to-blue-500 text-white">
            ...
            <CardDescription className="text-purple-100">Review and approve new account requests</CardDescription>
                          ...
                          <TableCell>
                            {request.referredBy ? (
                              <div className="flex items-center space-x-2">
                                ...
                                <div>
                                  <div className="text-sm font-medium">{getReferrerName(request.referredBy)}</div>
                                  <div className="text-xs text-gray-500">{request.referralCode}</div>
                                </div>
                              </div>
                            ) : (
            ...
            <div className="space-y-4">
              <div className="bg-gray-50 p-3 rounded-lg">
                ...
                {selectedRequest?.referredBy && (
                  <p className="text-sm">
                    <strong>Referred by:</strong> {getReferrerName(selectedRequest.referredBy)} (
...
export default function AdminSignupRequestsPage() {
  return (
    <ProtectedRoute requiredRole="admin">
      <AdminSignupRequestsPageContent />
    </ProtectedRoute>
  )
}
